...
def solver(fs, gs, tolerance=1e-10, max_iter=1000, first_order=False):
    '''
...
#     max_iter = 1000
    ...
    for counter in np.arange(max_iter):
        XYt = np.matmul(X.reshape((-1,1)), Y.reshape((1,-1)))
        x = fs / np.sum(Y / (1. + XYt), axis=1)
        y = gs / np.sum(X / (1. + XYt.transpose()), axis=1)
...
def solver_newton(fs, gs, tolerance=1e-10, max_iter=1000):
    '''
    Numerically solve the system of nonlinear equations
    we encounter when solving for the Lagrange multipliers
    ...
    http://homepage.divms.uiowa.edu/~atkinson/ftp/ENA_Materials/Overheads/sec_7-3.pdf
    https://www.jstor.org/stable/2156481?seq=4#page_scan_tab_contents

    '''

    def jacobian(X, Y):
        J = np.zeros((len(X) + len(Y), len(X) + len(Y)))

        for i in np.arange(len(X)):
            J[i, i] = np.sum(Y / np.power(1 + X[i]*Y, 2))
            J[i, len(X):] = X[i] / np.power(1 + X[i]*Y, 2)

        for i in np.arange(len(Y)):
            J[len(X) + i, :len(X)] = Y[i] / np.power(1 + X*Y[i], 2)
            J[len(X) + i, len(X) + i] = np.sum(X / np.power(1 + X*Y[i], 2))
    ...
    for counter in range(max_iter):
        ...
        X_eq = np.sum( XYt / (1. + XYt), axis=1) - fs
        Y_eq = np.sum( XYt.transpose() / (1. + XYt.transpose()), axis=1) - gs

        F = np.concatenate([X_eq, Y_eq])
        print 'F' , F
        J = jacobian(X, Y)
        print 'J', J

        J_inv_F = -np.matmul(np.linalg.inv(J), F.reshape((-1,1)))
        print 'J_inv_F' , J_inv_F.squeeze()
        var_next = var_eq + J_inv_F.squeeze()
#         print 'var_eq' , var_eq.shape
#         print 'var_next' , var_next.shape

        x = var_next[:len(X)]
        y = var_next[len(X):]

        # L_oo
        change = np.mean(np.abs(F))

        if counter % 1 == 0:
            print 'counter=%d, mean value=%f' % (counter, change)
