...
parser.add_argument('--dataset', nargs='?', default='alt.atheism',
                    help='dataset name')
parser.add_argument('--subset', nargs='?', default=1.0, type=float,
                    help='fraction of affiliations to use')
...
for dataset in os.listdir('/Users/tradergllc/rsi/datasets/newsgroups/csv/test/'):
    auc_base_list = []
    auc_ergm_list = []
    ...
    if dataset[0] != '.':
        for t in range(args.trials):
            path = os.path.join('/Users/tradergllc/rsi/datasets/newsgroups/csv/test',
                dataset, 'data%d.mtx' % (t + 1))
            adj_mx = mmread(path).toarray()

            n_subset = int(adj_mx.shape[1] * args.subset)
            subset_choice = np.random.choice(range(adj_mx.shape[1]), n_subset, replace=False)
            adj_mx = adj_mx[:, subset_choice]
            print adj_mx.shape
